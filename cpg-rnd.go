...
type cmpPtnRndFuncInst struct {
	...
	resp map[InEdgeStruct][]rndFuncOut
}
...
func createRndFuncInst(cpInstName string, fnc *Func, paramStr string) *cmpPtnRndFuncInst {
	...
	rfi.resp = make(map[InEdgeStruct][]rndFuncOut)
...
func (cprfi *cmpPtnRndFuncInst) funcDelay(msg *cmpPtnMsg) float64 {
	// look up the execution time dependent on this
	// function type and the message (potentially type and length)
	delay := funcExecTime(cprfi, msg)
...
func (cprfi *cmpPtnRndFuncInst) funcExec(msg *cmpPtnMsg) []*cmpPtnMsg {
