...
type CmpPtnMsg struct {
    ExecID    int    // initialize when with an initating comp pattern message.  
                     // Carried by every resulting message.
    FlowID    int    // identity of flow when message involves flows
    ClassID   int    // identity of flow class 
    MeasureID int    // when carrying a measure, the identity of that measure
    
    PrevCPID  int    // ID of the CmpPtn through which the message most recently passed
    ...
    RtnMsgType string // used on service calls

    Rate       float64 // when non-zero, a Flow rate
...
**pces** and **mrnes** support a network object called a *Flow*, which is traffic whose description is given in terms of average bitrate rather than a particular packet.  A *CmpPtnMsg* can carry information about a *Flow* in particular an edge where a bitrate change is propogated through the network.   Flows have identities, *FlowID* is a unique id given when a flow is created.   Flows get different levels of priority when competing for resources, the *ClassID* identifies the flow's priority class.  The larger the class id, the greater the priority.
...
*MsgLen* carries the number of bytes in the *CmpPtn* frame, and *PckLen* carries the number of bytes in the data frame, the latter being a parameter used when the execution time of a function processing the message is determined.
...
```
 ...
 38     }
 39     methods.Start(evtMgr, cpfi, methodCode, cpm)
 40     return nil
 41 }
```
...
Reaching line 39, the *Start* entry of the found struct is understood to be a function with a signature of

```
func(*evtm.EventMgr, *CmpPtnFuncInst, *CmpPtnMsg)
```

The functions called through line 39 are so-called 'Entry' functions for the different function classes.   Text description of their operation are given later when we speak to each class.
