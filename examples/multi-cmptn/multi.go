...
func main() {
	...
	// bring in RSA chain
	RSAChain, cpFound := cpPrbDict.RecoverCompPattern("RSAChain", "simple-RSA-chain")
	// fill in a Name, key used to reference this comp pattern when reading in the model

	if !cpFound {
		fmt.Println("could not recover comp pattern simple-RSA-chain from template file")
		os.Exit(1)
	}
	RSAChain.Name = "simple-RSA-chain"

	// bring in initialization for RSA
	RSACPInit, cpInit := cpInitPrbDict.RecoverCPInitList("RSAChain", "simple-RSA-chain")
	if !cpInit {
		fmt.Println("could not recover comp pattern initialization for simple-stateful-test from template file")
		os.Exit(1)
	}

	// put RSA comp pattern and CP into output dictionaries
	cmptnDict.AddCompPattern(RSAChain, false, false)
	cpInitDict.AddCPInitList(RSACPInit, false)
	...
	expCfg, xcpresent := expPrbDict.RecoverExpCfg("Topo")
	if !xcpresent {
	...
	param1 := mrnes.CreateExpParameter("Interface", wcAttrbs, "trace", "true")
	param2 := mrnes.CreateExpParameter("Switch", wcAttrbs, "trace", "true")
	param3 := mrnes.CreateExpParameter("Router", wcAttrbs, "trace", "true")
	param4 := mrnes.CreateExpParameter("Host", wcAttrbs, "trace", "true")
	...
	cmpMap := mrnb.CreateCompPatternMap("simple-RSA-chain")
